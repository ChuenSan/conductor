# conductor App（真终端 + 工作区/Tab/分屏 UI）Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** 在已验证的 ConductorCore（模型+持久化）和 libghostty spike 之上，做出一个可启动的单窗口 macOS app：左侧工作区栏 + 顶部 Tab + tab 内自由分屏，每个分屏叶子是一个**真 libghostty 终端**，支持新建/分屏/关闭/切换焦点，并把布局结构持久化。

**Architecture:** 分两段。**Phase A**：把"命令→状态变化+副作用"的纯逻辑下沉进 ConductorCore（reducer 模式：`apply(command, store) -> (newStore, [SessionEffect])`），并提供一个泛型 `SessionRegistry`（PaneID→TerminalSurface），全部用 `FakeSurface` 走 TDD。**Phase B**：新建 `ConductorApp` 可执行目标，把 spike 一般化为 `GhosttySurface`（含运行时 action 路由 + host NSView），用 SwiftUI 外壳 + 桥接 AppKit（`NSSplitView`、`NSViewRepresentable`）渲染 ConductorCore 的 `SplitNode` 树，命令经 Phase A 的 reducer 驱动。Phase B 是集成层，**靠 `swift build` + 运行 + 肉眼验证**，并大量复用 Conductor 的 `Apps/Conductor/Sources/Conductor/Terminal/` 已验证代码。

**Tech Stack:** Swift 6 / SwiftPM；ConductorCore（纯 Swift，XCTest）；SwiftUI + AppKit 桥接；预编译 GhosttyKit（libghostty C API，已 vendored）。
...
- 已落地的 spike：`Sources/ConductorSpike/`（最小可用的运行时 + host view，Phase B 的起点）
...
```
Sources/ConductorCore/                      (Phase A 新增/修改 — 纯 Swift)
...
Sources/ConductorApp/                       (Phase B 新增 — 可执行目标)
...
> 备注：Phase B 完成后，旧的 `Sources/ConductorSpike/` 在最后一个任务中删除（它已完成证明使命）。

---

# Phase A — ConductorCore 纯逻辑（TDD，全代码）
...
**Files:**
- Modify: `Sources/ConductorCore/TerminalSurface.swift`
- Modify: `Tests/ConductorCoreTests/FakeSurface.swift`
- Modify: `Tests/ConductorCoreTests/TerminalSurfaceTests.swift`

- [ ] **Step 1: 改协议**

把 `Sources/ConductorCore/TerminalSurface.swift` 整文件替换为：
...
把 `Tests/ConductorCoreTests/FakeSurface.swift` 整文件替换为：
...
把 `Tests/ConductorCoreTests/TerminalSurfaceTests.swift` 整文件替换为：
...
Expected: PASS（2 tests）。再跑 `swift test` 确认其余仍绿（ConductorSpike 不在 test target，不受影响）。
...
**Files:**
- Create: `Sources/ConductorCore/SessionEffect.swift`
- Test: （随 Task 3 一起测）

- [ ] **Step 1: 实现**

`Sources/ConductorCore/SessionEffect.swift`:
...
```bash
git add Sources/ConductorCore/SessionEffect.swift
...
**Files:**
- Create: `Sources/ConductorCore/WorkspaceCommand.swift`
- Test: `Tests/ConductorCoreTests/WorkspaceCommandTests.swift`

- [ ] **Step 1: 写失败测试**

`Tests/ConductorCoreTests/WorkspaceCommandTests.swift`:
...
`Sources/ConductorCore/WorkspaceCommand.swift`:
...
```bash
git add Sources/ConductorCore/SessionEffect.swift Sources/ConductorCore/WorkspaceCommand.swift Tests/ConductorCoreTests/WorkspaceCommandTests.swift
...
**Files:**
- Create: `Sources/ConductorCore/SessionRegistry.swift`
- Test: `Tests/ConductorCoreTests/SessionRegistryTests.swift`

- [ ] **Step 1: 写失败测试**

`Tests/ConductorCoreTests/SessionRegistryTests.swift`:
...
`Sources/ConductorCore/SessionRegistry.swift`:
...
```bash
git add Sources/ConductorCore/SessionRegistry.swift Tests/ConductorCoreTests/SessionRegistryTests.swift
...
## Task 5: 新建 ConductorApp 可执行目标 + 空窗口

**Files:**
- Modify: `Package.swift`（加 `ConductorApp` executableTarget，依赖 `ConductorCore` + `GhosttyKit`，linkerSettings 同 ConductorSpike）
- Create: `Sources/ConductorApp/main.swift`（AppKit 入口 + 一个空 `NSWindow`，沿用 ConductorSpike/main.swift 的 `MainActor.assumeIsolated` + AppDelegate 结构，标题 "conductor"）
- Create: `Sources/ConductorApp/UI/RootView.swift`（先放一个占位 `NSView`/空 SwiftUI `Text("conductor")`）

**适配点：** target 设 `.swiftLanguageMode(.v5)`（同 ConductorSpike，避免 C 回调并发摩擦）。

- [ ] Step 1: 在 Package.swift 的 `products` 加 `.executable(name: "ConductorApp", targets: ["ConductorApp"])`，`targets` 加 `ConductorApp`（dependencies: `["ConductorCore", "GhosttyKit"]`，swiftSettings `[.swiftLanguageMode(.v5)]`，linkerSettings 同 ConductorSpike 的 6 个：c++/AppKit/Carbon/IOSurface/Metal/QuartzCore）。
- [ ] Step 2: 写 `main.swift`（复制 `Sources/ConductorSpike/main.swift`，contentView 暂时 `NSView()`，标题改 "conductor"）。
- [ ] Step 3: `swift build --product ConductorApp`，预期 `Build complete!`（含无害 ImGui 链接告警）。
- [ ] Step 4: 运行 `.build/debug/ConductorApp`，肉眼确认弹出空窗口。Ctrl-C/关窗退出。
- [ ] Step 5: 提交 `feat(app): scaffold ConductorApp executable with empty window`。

## Task 6: GhosttyRuntime（含 action_cb 路由）

**Files:**
- Create: `Sources/ConductorApp/Terminal/GhosttyRuntime.swift`

**复用：** 以 `Sources/ConductorSpike/GhosttyRuntime.swift` 为基础，把 `action_cb` 从 `{ _, _, _ in false }` 升级为：按 `target.tag == GHOSTTY_TARGET_SURFACE` 取 `target.target.surface`，用 `ghostty_surface_userdata` 取回 `GhosttySurface`（见 Task 7），分派 `GHOSTTY_ACTION_SET_TITLE`/`GHOSTTY_ACTION_PWD`/`GHOSTTY_ACTION_SHOW_CHILD_EXITED`/`GHOSTTY_ACTION_CLOSE_TAB` 到对应回调。**直接参照** Conductor 的 `GhosttyAppRuntime.handleAction`（只保留这 4 个 case，其余 `default: return false`）。`wakeup_cb`/tick/`ensureStarted` 沿用 spike。

- [ ] Step 1: 写文件（singleton `GhosttyRuntime.shared`，`ensureStarted()` 同 spike，新增 `action_cb` 路由 title/pwd/exit）。
- [ ] Step 2: `swift build --product ConductorApp`，预期 Build complete（此任务可能引用 Task 7 的 GhosttySurface，与 Task 7 一并编译通过；若单独构建报缺 GhosttySurface，先占位再于 Task 7 接上）。
- [ ] Step 3: 提交 `feat(app): GhosttyRuntime with per-surface action routing`。

## Task 7: GhosttySurface + TerminalHostView

**Files:**
- Create: `Sources/ConductorApp/Terminal/TerminalHostView.swift`（NSView，CAMetalLayer 背衬 + 键鼠转发；改自 `Sources/ConductorSpike/SpikeTerminalView.swift`，但把"创建 surface/几何/输入"拆给 GhosttySurface，view 只转发事件给 `weak var surface: GhosttySurface?`，参照 Conductor 的 TerminalHostView 精简版）
- Create: `Sources/ConductorApp/Terminal/GhosttySurface.swift`（`final class GhosttySurface: ConductorCore.TerminalSurface`，持有 `ghostty_surface_t` + `TerminalHostView`，实现 `start(cwd:)`=创建 surface（参照 spike 的 createSurface，command `/bin/zsh`、working_directory=cwd、userdata=`Unmanaged.passRetained(self)`）、`focus()`、`close()`，并暴露 `hostView`；`onTitleChange/onCwdChange/onExit` 由 GhosttyRuntime 的 action 路由触发；静态 `fromGhosttySurface(_:)` 用 userdata 取回实例）
...
- [ ] Step 3: `swift build --product ConductorApp`，预期 Build complete。
...
**Files:**
- Create: `Sources/ConductorApp/UI/TerminalPaneView.swift`（`NSViewRepresentable`/或直接 NSView 包装，把某 `PaneID` 对应 GhosttySurface 的 hostView 放进去；从 `SessionRegistry` 取 surface）
- Create: `Sources/ConductorApp/UI/SplitContainerView.swift`（输入一棵 `SplitNode` + 一个"取 pane 视图"的闭包，递归构建 `NSSplitView`：`.leaf` → pane 视图；`.split(axis,ratio,first,second)` → 一个 `NSSplitView`（`.isVertical = axis == .vertical`），按 ratio 设分隔位置，子项递归）
...
- [ ] Step 3: `swift build --product ConductorApp`，Build complete。
...
**Files:**
- Create: `Sources/ConductorApp/AppCoordinator.swift`（`@MainActor` 持有 `var store: WorkspaceStore`、`SessionRegistry`（工厂创建 GhosttySurface）、当前布局视图的刷新闭包；`run(_ command: WorkspaceCommand)`：`let effects = command.apply(to: &store); registry.apply(effects); rebuildActiveTabView()`；ID 用 `UUID().uuidString` 生成）
- Modify: `Sources/ConductorApp/main.swift`（初始化 coordinator，seed 一个工作区 w1=当前目录或 $HOME，一个 tab 一个 pane；窗口 contentView = 当前 tab 的 SplitContainerView；安装键位）
...
**Files:**
- Create: `Sources/ConductorApp/UI/TabBarView.swift`（SwiftUI 自绘横向 tab 条：当前工作区的 tabs，点击 → `coordinator.run(.selectTab(id))`，`+` → `.newTab`；active tab 高亮）
- Modify: `Sources/ConductorApp/UI/RootView.swift`（顶部 TabBar + 下方 SplitContainer）
- Create: `Sources/ConductorApp/Support/AppStyle.swift`（配色/间距常量）
...
**Files:**
- Create: `Sources/ConductorApp/UI/SidebarView.swift`（SwiftUI 自绘左侧工作区列表：列出 `store.workspaces`，点击切换 active workspace，`+` 选目录新建工作区（`NSOpenPanel` 选目录 → 新 Workspace，path=选中目录，含一个 tab 一个 pane）；active 高亮）
- Modify: `Sources/ConductorApp/AppCoordinator.swift`（加 `selectWorkspace(WorkspaceID)` 与 `addWorkspace(path:)`：用 `WorkspaceStore.upsert/activeWorkspace`，切换时 rebuild 并 focus 其 active tab 的 pane）
...
**Files:**
- Delete: `Sources/ConductorSpike/`（移除目标：Package.swift 去掉 ConductorSpike product/target）
- Modify: `Sources/ConductorApp/Support/AppStyle.swift` + 各 UI（间距/配色/焦点高亮/hover 打磨一遍）
- Create: `README.md`（构建步骤：`./Scripts/prepare-ghosttykit.sh` → `swift build` → `swift run ConductorApp`）

- [ ] Step 1: 删除 ConductorSpike 目标与目录。
- [ ] Step 2: 样式打磨（active pane 焦点环、tab/侧栏 active 态、配色统一）。
- [ ] Step 3: 写 README。
- [ ] Step 4: `swift build` 全量 + `swift test`（ConductorCore 全绿）+ 运行 ConductorApp 完整冒烟（工作区/tab/分屏/持久化）。
...
- 持久化 B（只存结构、原 cwd 起空白 shell、坏文件兜底）→ Task 12 ✓（用 ConductorCore 已有 StateStore/CwdResolver）
- 默认键位 → Task 9 ✓
- 自绘样式 → Task 10/11/13 ✓
