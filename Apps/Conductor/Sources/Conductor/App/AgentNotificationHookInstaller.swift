...
enum AgentNotificationHookInstaller {
    private static let trustBegin = "# BEGIN CONDUCTOR AGENT HOOK TRUST"
    private static let trustEnd = "# END CONDUCTOR AGENT HOOK TRUST"

    static func installAll(bridgePath: String) -> [Result<String, Error>] {
        AgentHookProvider.allCases.map { provider in
            Result {
                try install(provider: provider, bridgePath: bridgePath)
            }
        }
    }
    ...
    private static func installCodex(bridgePath: String) throws -> String {
        ...
        let codexHome = resolvedConfigDirectory(defaultRelativePath: ".codex", environmentKey: "CODEX_HOME")
        try fileManager.createDirectory(at: codexHome, withIntermediateDirectories: true)

        let hooksURL = codexHome.appendingPathComponent("hooks.json")
        var root = try readJSONObject(at: hooksURL)
        var hooks = root["hooks"] as? [String: Any] ?? [:]
        hooks = removingOwnedHooks(in: hooks, provider: .codex)

        for (eventName, action) in [
            ...
            groups = groups.compactMap { removingOwnedHooks(from: $0, provider: .codex) }
            groups.append([
                "hooks": [
                    [
                        "type": "command",
        ...
        let updatedConfig = installingCodexTrustEntries(
            in: enablingCodexHooks(in: existingConfig),
            hooks: hooks,
            hooksURL: hooksURL
        )
    ...
    private static func installClaudeCode(bridgePath: String) throws -> String {
        let fileManager = FileManager.default
        let claudeHome = resolvedConfigDirectory(defaultRelativePath: ".claude", environmentKey: "CLAUDE_CONFIG_DIR")
        ...
        var hooks = root["hooks"] as? [String: Any] ?? [:]
        hooks = removingOwnedHooks(in: hooks, provider: .claudeCode)
        ...
        for (eventName, action) in [
            ...
            var groups = hooks[eventName] as? [[String: Any]] ?? []
            groups = groups.compactMap { removingOwnedHooks(from: $0, provider: .claudeCode) }
            groups.append([
                "hooks": [
                    [
                        "type": "command",
    ...
    private static func resolvedConfigDirectory(defaultRelativePath: String, environmentKey: String) -> URL {
        ...
        return FileManager.default.homeDirectoryForCurrentUser.appendingPathComponent(defaultRelativePath, isDirectory: true)
    }
    ...
    private static func readJSONObject(at url: URL) throws -> [String: Any] {
        ...
        guard let object = try JSONSerialization.jsonObject(with: data) as? [String: Any] else {
            throw NSError(
                ...
                code: 1,
                userInfo: [NSLocalizedDescriptionKey: "\(url.lastPathComponent) 不是有效 JSON，未自动修改。"]
            )
        }
        return object
    }

    private static func removingOwnedHooks(in hooks: [String: Any], provider: AgentHookProvider) -> [String: Any] {
        var nextHooks = hooks
        for (eventName, rawGroups) in hooks {
            guard let groups = rawGroups as? [[String: Any]] else { continue }
            let cleanedGroups = groups.compactMap { removingOwnedHooks(from: $0, provider: provider) }
            if cleanedGroups.isEmpty {
                nextHooks.removeValue(forKey: eventName)
            } else {
    ...
    private static func isOwnedCommand(_ command: String, provider: AgentHookProvider) -> Bool {
        ...
        guard lowercased.contains("conductor") || normalized.contains("CONDUCTOR_") else { return false }
        switch provider {
        case .codex:
    ...
    private static func enablingCodexHooks(in content: String) -> String {
        ...
        for index in lines.indices {
            ...
            if trimmed == "[features]" {
                featuresStart = index
                featuresEnd = lines[(index + 1)...].firstIndex {
    ...
    private static func installingCodexTrustEntries(
        in content: String,
        hooks: [String: Any],
        hooksURL: URL
    ) -> String {
