...
extension TextSelectionManager {
    ...
    func getFillRects(in rect: NSRect, for textSelection: TextSelection) -> [CGRect] {
        ...
        if let finalEmptyLineRect = trailingEmptyLineFillRect(
            in: validTextDrawingRect,
            selectionRange: range
        ) {
            fillRects.append(finalEmptyLineRect)
        }
    ...
    /// `TextLineStorageRangeIterator` walks ranges by character offsets, so a final 0-length line produced by a
    /// trailing newline is skipped even when the selection reaches EOF. Draw that visual line explicitly.
    private func trailingEmptyLineFillRect(in rect: NSRect, selectionRange range: NSRange) -> CGRect? {
        guard let layoutManager,
              range.length > 0,
              range.max == layoutManager.lineStorage.length,
              let finalLine = layoutManager.lineStorage.last,
              finalLine.range.isEmpty,
              finalLine.range.location == layoutManager.lineStorage.length else {
            return nil
        }
    ...
    private func getFillRects(
        ...
        for fragmentPosition in linePosition.data.lineFragments {
            ...
            let selectionEndsOnTrailingEmptyLine = layoutManager.lineStorage.last?.range.isEmpty == true
                && range.max == layoutManager.lineStorage.length
            ...
            if endOfLine && !(endOfDocument && !emptyLine && !selectionEndsOnTrailingEmptyLine) {
                maxRect = CGRect(
                    x: rect.maxX,
