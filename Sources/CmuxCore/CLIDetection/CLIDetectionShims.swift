...
// 打日志；cmux 不想引入 CodexBar 的整套日志子系统，这里用 os.Logger 做一个
...
public struct CodexBarLogger: Sendable {
    ...
    init(category: String) {
        self.logger = Logger(subsystem: "com.cmux.cli-detection", category: category)
    }
...
public enum ClaudeStatusProbe {
    public static func preparedProbeWorkingDirectoryURL() -> URL {
        let directory = FileManager.default.temporaryDirectory
            .appendingPathComponent("cmux-cli-probe", isDirectory: true)
