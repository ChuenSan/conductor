...
enum CLIDetectionStore {
    ...
    static func load() -> CLIDetectionCache? {
        guard let data = try? Data(contentsOf: fileURL),
              let cache = try? JSONDecoder.conductor.decode(CLIDetectionCache.self, from: data)
        else { return nil }
    ...
    static func save(_ tools: [CLIToolStatus], detectedAt: Date = Date()) -> CLIDetectionCache {
        ...
        if let data = try? JSONEncoder.conductor.encode(cache) {
...
private extension JSONDecoder {
    static let conductor: JSONDecoder = {
        let d = JSONDecoder()
...
private extension JSONEncoder {
    static let conductor: JSONEncoder = {
        let e = JSONEncoder()
