...
/// 目录：~/Library/Application Support/conductor/scrollback/<paneID>.txt
@MainActor
enum ScrollbackStore {
    ...
    static func ensureWrapperScript() -> String? {
        ...
        let script = """
        #!/bin/sh
        # conductor 内容恢复：回放上次会话文本，再进入真正的 shell。由 conductor 自动生成/覆盖。
        f="$CONDUCTOR_RESTORE_FILE"; s="$CONDUCTOR_RESTORE_SHELL"
        unset CONDUCTOR_RESTORE_FILE CONDUCTOR_RESTORE_SHELL
        if [ -n "$f" ] && [ -f "$f" ]; then
          cat -- "$f"
          rm -f -- "$f"
        fi
        ...
        do {
            ...
            NSLog("[conductor] restore-shell.sh write failed: \(error)")
            return nil
        }
    }
}
