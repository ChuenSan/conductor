...
/// 实现 CmuxCore 的引擎无关 `TerminalSurface` 生命周期协议。几何/输入逻辑也在此（host view 只转发事件）。
@MainActor
final class GhosttySurface: TerminalSurface {
    ...
    // CmuxCore.TerminalSurface 回调（由 SessionRegistry 注入；运行时 action 路由触发）
    ...
    func attachIfPossible() {
        ...
        if let restoreFile = restoreContentFile,
           ...
           let wrapper = ScrollbackStore.ensureWrapperScript() {
            command = wrapper
            envPairs = [("CMUX_RESTORE_FILE", restoreFile), ("CMUX_RESTORE_SHELL", shell)]
        }
        ...
        guard let surface else {
            userdata.release()
            NSLog("[cmux] ghostty_surface_new failed")
            return
        }
