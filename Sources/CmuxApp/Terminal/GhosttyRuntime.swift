...
final class GhosttyRuntime {
    ...
    func ensureStarted() {
        ...
        guard let created = ghostty_app_new(&rc, config) else {
            NSLog("[cmux] ghostty_app_new failed")
        ...
        NSLog("[cmux] ghostty runtime started")
    }
    ...
    static func makeConfig(from appConfig: AppConfig) -> ghostty_config_t? {
        ...
        text.withCString {
            ghostty_config_load_string(config, $0, UInt(text.utf8.count), "cmux")
        }
    ...
    static func ghosttyConfigText(from config: AppConfig) -> String {
        ...
        for (key, rawValue) in config.ghosttyOverrides.sorted(by: { $0.key < $1.key }) {
            ...
            guard CmuxGhosttyConfigCatalog.knownKeySet.contains(key), !value.isEmpty else { continue }
            set(key, value)
        }
