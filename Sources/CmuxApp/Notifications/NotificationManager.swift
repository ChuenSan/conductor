...
/// 这时用户可在「系统设置 › 通知 › cmux」手动打开，即可恢复点击跳转。
@MainActor
final class NotificationManager: NSObject, UNUserNotificationCenterDelegate {
    ...
    func configure() {
        guard Bundle.main.bundleIdentifier != nil else {
            hasBundle = false
            NSLog("[cmux] 无 bundle id（裸二进制），通知走 osascript 回退。打包成 cmux.app 可启用点击跳转。")
            return
        }
        hasBundle = true
        ...
        center.requestAuthorization(options: [.alert, .sound]) { [weak self] granted, error in
            if let error { NSLog("[cmux] 通知授权请求出错：\(error.localizedDescription)") }
    ...
    /// 打开「系统设置 › 通知」，引导用户为 cmux 开启通知（恢复点击跳转）。
