...
struct CmuxFloatingPanel: ViewModifier {
...
extension View {
    /// 真浮层（透出桌面）：命令面板、弹出菜单等。
    func cmuxFloatingPanel(cornerRadius: CGFloat = Radius.xl) -> some View {
        modifier(CmuxFloatingPanel(cornerRadius: cornerRadius, blending: .behindWindow))
    }

    /// 窗内材质（融入主窗）：停靠的设置 / 检查器面板。
    func cmuxInWindowPanel(cornerRadius: CGFloat = 0) -> some View {
        modifier(CmuxFloatingPanel(cornerRadius: cornerRadius, blending: .withinWindow))
    }
}
