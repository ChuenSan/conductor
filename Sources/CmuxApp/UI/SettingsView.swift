...
struct SettingsView: View {
    ...
    private var ghosttySection: some View {
        VStack(spacing: 16) {
            ...
            ForEach(CmuxGhosttyConfigCatalog.productGroups) { group in
...
private struct GhosttyConfigGroupHeader: View {
    let group: CmuxGhosttyConfigGroup
    var first = false
...
private struct GhosttyOverrideRow: View {
    ...
    private var copy: CmuxGhosttyConfigCopy { CmuxGhosttyConfigCatalog.copy(for: key) }
...
private struct GhosttyPercentControl: View {
    ...
    let range: ClosedRange<Double>
    let defaultValue: Double
    let format: CmuxGhosttyPercentFormat
    let setValue: (String) -> Void
