...
public enum ProviderVersionDetector {
    ...
    public static func grokVersion() -> String? {
        let env = ProcessInfo.processInfo.environment
        guard let path = BinaryLocator.resolveGrokBinary(env: env, loginPATH: nil)
            ?? TTYCommandRunner.which("grok") else { return nil }
    ...
    /// 通用版本探测：从 PATH 解析 `command`，依次尝试常见 version 参数。
    public static func genericVersion(command: String) -> String? {
