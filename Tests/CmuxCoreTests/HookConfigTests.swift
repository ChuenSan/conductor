...
final class HookConfigTests: XCTestCase {
    private func tempFile() -> URL {
        FileManager.default.temporaryDirectory
            .appendingPathComponent("cmux-hookcfg-\(UUID().uuidString).json")
    }
    ...
    func testAddReadRemovePreservesOtherKeys() throws {
        ...
        try doc.addCommand(event: "Stop", command: "echo hi #cmux:test", timeout: 5000)
        let entries = doc.entries()
        XCTAssertEqual(entries.count, 2)
        XCTAssertTrue(entries.contains { $0.command == "echo hi #cmux:test" && $0.managedByCmux })
        XCTAssertTrue(entries.contains { $0.command == "existing" && !$0.managedByCmux })
        ...
        // 重复添加不增加
        try doc.addCommand(event: "Stop", command: "echo hi #cmux:test")
        XCTAssertEqual(doc.entries().count, 2)

        // 按哨兵移除，只移我们的
        let removed = try doc.removeCommands(containing: "#cmux:test")
    ...
    func testAddToFreshFile() throws {
        ...
        try doc.addCommand(event: "Stop", command: "do-thing #cmux:x")
        XCTAssertEqual(doc.entries().count, 1)
